...
public class Comprador {
    ...
    public Comprador(Moneda m, int cualProducto, Expendedor exp) throws PagoIncorrectoException, NoHayProductoException, PagoInsuficienteException {
        ...
        if ((p = exp.getProducto()) != null) {
            ...
            exp.comprarProducto(m, cualProducto);  //Compra producto
            if ((p = exp.getProducto()) != null) {

                sonido = p.consumir();
                serieCom = p.getSerie(p);
    ...
    /**
     * @return Retorna un entero que indica el numero de serie del producto
     */

    public int serieCompra() {
        return serieCom;
    }
}
